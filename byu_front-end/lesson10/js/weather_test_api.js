...
fetch(apiURL)
  ...
  .then((pepe) => {
    console.pepe
    document.getElementById('current-temp').textContent = pepe.main.temp;

    //icons

    const imagesrc = 'https://openweathermap.org/img/w/' + pepe.weather[0].icon + '.png';  // note the concatenation
const desc = pepe.weather[0].description;  // note how we reference the weather array
